...
class Ggowl_Featurred_Image_Getter extends Group_Control_Image_Size
{
     ...
     */
    public static function get_attachment_image_html_generator($attachment_id, $settings, $image_size_key = 'image', $image_key = null,$extra = '')
    {
        if (!$image_key) {
            $image_key = $image_size_key;
        }
        if(!empty($extra) && is_bool($extra['url']) )  {
        ...
        if (!empty($image['id']) && in_array($size, $image_sizes)) {
            ...
            $html .= wp_get_attachment_image($attachment_id, $size, false, $image_attr);
        } else {

            $image_src = self::get_attachment_image_src($attachment_id, $image_size_key, $settings);
    ...
    protected function init_fields()
    {
        $fields         = [];
        $fields['size'] = [
            'label'       => esc_html_x('Image Size', 'Image Size Control', 'tmpenvo'),
            'type'        => Controls_Manager::SELECT,
            'label_block' => false,
        ];
        $fields['custom_dimension'] = [
            'label'       => esc_html_x('Image Dimension', 'Image Size Control', 'tmpenvo'),
            'type'        => Controls_Manager::IMAGE_DIMENSIONS,
            'description' => esc_html__('You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'tmpenvo'),
            'condition'   => [
                'size' => 'custom',
            ],
            'separator'   => 'none',
    ...
    private function get_image_sizes()
    {
        ...
        $image_sizes['full'] = esc_html_x('Full', 'Image Size Control', 'tmpenvo');
        if (!empty($args['include']['custom']) || !in_array('custom', $args['exclude'])) {
            $image_sizes['custom'] = esc_html_x('Custom', 'Image Size Control', 'tmpenvo');
        }
        return $image_sizes;
    }
    /**
